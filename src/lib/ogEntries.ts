/**
 * ogEntries.ts. single source of truth for which pages get a
 * Satori-rendered Open Graph card, and what goes on it.
 *
 * Two consumers:
 *   - src/pages/og/[...slug].png.ts  → renders one PNG per entry
 *   - src/layouts/BaseLayout.astro   → decides whether a page's
 *     <meta property="og:image"> may point at /og/<slug>.png
 *
 * They MUST agree. Before this module existed the layout mapped every
 * page path → /og/<path>.png unconditionally, so the ~3,000 ward pages
 * (and every page during a BUILD_OG=0 build) advertised an OG URL that
 * was never generated. Cloudflare Pages answers those with the HTML
 * 404 body under a 200, so crawlers got text/html where they expected
 * image/png and every social preview broke.
 *
 * Gated behind BUILD_OG=1. When unset, buildEntries() returns [] —
 * which now also means the layout falls back to the static card for
 * every page, so an iteration build never advertises a card it did not
 * render.
 *
 * Slug shape:
 *   index                              → homepage
 *   forecasts/general-election         → GE forecast hub
 *   by-elections/makerfield            → Makerfield by-election
 *   polling                            → /polling/
 *   past-results                       → /past-results/
 *   councils                           → /councils/
 *   seats/parliament/<slug>            → each of 650 PCONs
 *   seats/<council>                    → ~360 LADs
 *
 * Ward pages (~3,000) are skipped in v1 to keep build time under
 * 12 minutes; they fall back to the static card.
 */
import {
  loadGePredictions,
  loadIdentity,
...
import { UK_WESTMINSTER_2026_APRIL_AVERAGE } from "./nationalPolling.js";
import type { OgShare } from "./ogRenderer";

export const BUILD_OG = process.env.BUILD_OG === "1";
...
/** Every card this build will emit. Empty when BUILD_OG is unset. */
export const OG_ENTRIES: OgEntry[] = buildEntries();

/** Slug lookup for the layout. Empty when BUILD_OG is unset. */
export const OG_CARD_SLUGS: ReadonlySet<string> = new Set(OG_ENTRIES.map((e) => e.slug));

/**
 * Page pathname → card slug, matching ogPathFor() in BaseLayout.
 * "/" → "index"; "/seats/parliament/wigan/" → "seats/parliament/wigan".
 */
...
/** True when this build actually rendered a card for this page. */
export function hasOgCard(pathname: string): boolean {
  return OG_CARD_SLUGS.has(ogSlugForPathname(pathname));
}
